...
# Configuration
BACKEND_URL = (
    "http://127.0.0.1:8000"  # Consider making this configurable via env or query param
)
...
def main():
    ...
    st.set_page_config(page_title="47Chat Orchestrator", page_icon="🤖", layout="wide")
                ...
                with st.spinner("Uploading and processing files..."):
                    ...
                    try:
                        response = requests.post(
                            f"{BACKEND_URL}/upload/", files=files_data
                        )
                        if response.status_code == 200:
            ...
            if question.strip():
                with st.spinner("Running multi-agent orchestration..."):
                    try:
                        ...
                        response = requests.post(
                            f"{BACKEND_URL}/orchestrate/", json=payload
                        )
            ...
            if len(st.session_state.discussions) > 1:
                with st.expander("📝 Discussion History", expanded=False):
                    for i, discussion in enumerate(st.session_state.discussions[1:], 1):
                        st.write(
                            f"**Discussion {i}:** {discussion.get('user_prompt', 'Unknown')[:50]}..."
                        )
