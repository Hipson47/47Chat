...
class TestRAG(unittest.TestCase):
    ...
    def test_1_upload(self):
        ...
        with open(self.sample_doc_path, "rb") as f:
            files = {
                "files": (os.path.basename(self.sample_doc_path), f, "text/markdown")
            }
    ...
    def test_2_retrieve(self):
        ...
        self.assertTrue(
            os.path.exists("rag_store.faiss"), "FAISS index file should exist"
        )
        self.assertTrue(os.path.exists("rag_chunks.json"), "Chunks file should exist")

        results = rag.retrieve("software development", k=1)
        self.assertGreaterEqual(
            len(results), 1, "Should retrieve at least one result from persisted store"
        )
